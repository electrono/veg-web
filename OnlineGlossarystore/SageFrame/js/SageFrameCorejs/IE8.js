...
(function() {
    IE7 = { toString: function() { return "IE7 version 2.0 (beta3)" } };
    ...
    if (m < 5.5) {
        ...
        Function.prototype.call = function(a) { return this.apply(a, bP.apply(arguments, [1])) };
    ...
    B.prototype.extend = function(a) { return bf(this, a) };
    var bz = "#";
    var V = "~";
    var cK = /\\./g ;
    var cL = /\(\?[:=!]|\[[^\]]+\]/g ;
    ...
    var H = B.extend({
            ...
            merge: function(a) { for (var b in a) this.add(b, a[b]) },
            toString: function() { return "(" + this[V].join(")|(") + ")" }
        }, {
            IGNORE: "$0",
            Item: B.extend({
    ...
    function i(c) {
        ...
        return String(c).replace(f, function(a, b) { return b < d.length ? d[b] : a })
    }
    ...
    function bg(c) {
        return c.replace(cO, function(a, b) { return bA[b - 1] })
    }

    ;

    function bW(c) {
        return "\x01" + bA.push(c.replace(cR, function(a, b) { return eval("'\\u" + "0000".slice(b.length) + b + "'") }).slice(1, -1).replace(cP, "\\'"))
    }
    ...
    var ce = H.extend({
        ...
        ignoreCase: true,
        escape: function(a) { return this.optimise(this.format(a)) },
        format: function(a) { return a.replace(cY, "$1").replace(cX, "$1 $2").replace(cW, "$1*$2") },
        optimise: function(a) { return this.sorter.exec(a.replace(cd, ">* ")) },
        unescape: function(a) { return bg(a) }
    });
    ...
    IE7.CSS = new(bZ.extend({
        parser: new bU, screen: "", print: "", styles: [], rules: [], pseudoClasses: m < 7 ? "first\\-child" : "",
        ...
        addEventHandler: function() { v.apply(null, arguments) },
        addFix: function(a, b) { this.parser.add(a, b) },
        addRecalc: function(c, d, f, g) {
            d = new RegExp("([{;\\s])" + c + "\\s*:\\s*" + d + "[^;}]*");
            var h = this.recalcs.length;
            if (g) g = c + ":" + g;
            this.addFix(d, function(a, b) { return (g ? b + g : a) + ";ie7-" + a.slice(1) + ";ie7_recalc" + h + ":1" });
        ...
        refresh: function() { this.styleSheet.cssText = J + this.screen + this.print },
        trash: function() {
    ...
    var bJ = B.extend({
        ...
        getText: function() {
            ...
            function s(a) {
                if (q.test(a)) return "all";
                else if (r.test(a)) return (k.test(a)) ? "all" : "screen";
                else if (k.test(a)) return "print"
            }
        ...
        toString: function() { return "@media " + this.media + "{" + this.cssText + "}" }
    });
    var F;
    var o = IE7.Rule = B.extend({
        ...
        toString: function() { return i("%1 {%2}", this.selectorText, this.cssText) }
    ...
    if (m < 7) {
        var Z = new A("hover", function(a) {
            var b = arguments;
            IE7.CSS.addEventHandler(a, m < 5.5 ? "onmouseover" : "onmouseenter", function() { Z.register(b) });
            IE7.CSS.addEventHandler(a, m < 5.5 ? "onmouseout" : "onmouseleave", function() { Z.unregister(b) })
        });
    ...
    IE7.CSS.addRecalc("[\\w-]+", "inherit", function(c, d) {
        ...
        for (var g = 0; g < f.length; g++) {
            var h = f[g].replace( /ie7\-|\s*:\s*inherit/g , "").replace( /\-([a-z])/g , function(a, b) { return b.toUpperCase() });
            c.runtimeStyle[h] = c.parentElement.currentStyle[h]
        }
    });
    IE7.HTML = new(bZ.extend({
        fixed: { }, init: U, addFix: function() { this.fixes.push(arguments) },
        apply: function() {
    ...
    new function(_) {
        ...
        IE7.hasLayout = m < 5.5 ? function(a) { return a.clientWidth } : function(a) { return a.currentStyle.hasLayout };
        ...
        var fixWidth = function(g) {
            ...
            var r = function(a, b) {
                ...
                var d = 0;
                if (G && !c) d += k(a) + u(a, "padding");
                else if (!G && c) d -= k(a) + u(a, "padding");
                return q(a, b) + d
            };
    ...
    if (m >= 5.5 && m < 7) {
        ...
        IE7.HTML.addRecalc("img,input", function(a) {
            ...
            bL(a);
            v(a, "onpropertychange", function() { if (!bn && event.propertyName == "src" && a.src.indexOf(bk) == -1) bL(a) })
        });
        var bn = false;
        v(window, "onbeforeprint", function() {
            bn = true;
    ...
    new function(_) {
        ...
        function _7(a, b) {
            ...
            if (!a.runtimeStyle.autoLeft && a.currentStyle.marginLeft == "auto" && a.currentStyle.right != "auto") {
                ...
                if (_2(a.offsetParent)) a.runtimeStyle.pixelLeft += c;
        ...
        ;
        var _9;
        bC(function() { if (!_9) _9 = setTimeout(_j, 0) })
    };
    ...
    function cq() {
        ...
        IE7.CSS.addFix( /(font(-size)?\s*:\s*)([\w.-]+)/ , function(a, b, c, d) { return b + (f[d] || d) });
        if (m < 6) {
            var h = /^\-/ , j = /(em|ex)$/i ;
            var q = /em$/i , r = /ex$/i ;
            D = function(a, b) {
            ...
            function u(a) {
                ...
                while (a != w) {
                    ...
                    if (c) {
                        if (q.test(c)) b *= parseFloat(c);
                        else if (M.test(c)) b *= (parseFloat(c) / 100);
                        else if (r.test(c)) b *= (parseFloat(c) / 2);
                        else {
                            k.style.fontSize = c;
                            return 1
                        }
    ...
    var bb = new A("focus", function(a) {
        ...
        IE7.CSS.addEventHandler(a, "onblur", function() { bb.unregister(b) });
    ...
    var bq = new A("active", function(a) {
        var b = arguments;
        IE7.CSS.addEventHandler(a, "onmousedown", function() { bq.register(b) })
    });
    v(document, "onmouseup", function() {
        var a = bq.instances;
    ...
    var bu = new A("indeterminate", function(a) {
        ...
        IE7.CSS.addEventHandler(a, "onpropertychange", function() {
            if (event.propertyName == "indeterminate") {
                if (a.indeterminate) bu.register(b);
                else bu.unregister(b)
            }
        });
        IE7.CSS.addEventHandler(a, "onclick", function() { bu.unregister(b) })
    });
    ...
    IE7.HTML.addRecalc("img", function(a) { if (a.alt && !a.title) a.title = "" });
    ...
    IE7.CSS.addFix( /opacity\s*:\s*([\d.]+)/ , function(a, b) { return "zoom:1;filter:Alpha(opacity=" + ((b * 100) || 1) + ")" });
    var cx = /^image/i ;
    ...
    (function() {
        ...
        bm = new RegExp(W(typeof IE7_PNG_SUFFIX == "string" ? IE7_PNG_SUFFIX : ".png") + "$", "i");
        w = document.body;
        t = G ? w : bx;
