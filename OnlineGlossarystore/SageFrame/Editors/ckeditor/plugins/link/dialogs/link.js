...
CKEDITOR.dialog.add('link', function(a) {
    var b = CKEDITOR.plugins.link, c = function() {
        ...
        if (H = G.match(e))
            if (y == 'encode') G = G.replace(l, function(ab, ac, ad) { return 'mailto:' + String.fromCharCode.apply(String, ac.split(',')) + (ad && w(ad)); });
            else if (y)
                G.replace(m, function(ab, ac, ad) {
                    if (ac == z.name) {
                        L.type = 'email';
        ...
        return L;
    }, q = function(E, F) { if (F[E]) this.setValue(F[E][this.id] || ''); }, r = function(E) { return q.call(this, 'target', E); }, s = function(E) { return q.call(this, 'adv', E); }, t = function(E, F) {
        if (!F[E]) F[E] = { };
        F[E][this.id] = this.getValue() || '';
    }, u = function(E) { return t.call(this, 'target', E); }, v = function(E) { return t.call(this, 'adv', E); };
    ...
    if (y && y != 'encode') {
        ...
        y.replace( /^([^(]+)\(([^)]+)\)$/ , function(E, F, G) {
            z.name = F;
            z.params = [];
            G.replace( /[^,\s]+/g , function(H) { z.params.push(H); });
        });
    }
    ...
    return {
        ...
        contents: [{
            ...
            label: D.info,
            title: D.info,
            elements: [{ id: 'linkType', type: 'select', label: D.type, 'default': 'url', items: [[D.toUrl, 'url'], [D.toAnchor, 'anchor'], [D.toEmail, 'email']], onChange: d, setup: function(E) { if (E.type) this.setValue(E.type); }, commit: function(E) { E.type = this.getValue(); } }, {
                type: 'vbox',
                id: 'urlOptions',
                children: [{
                    type: 'hbox',
                    ...
                    children: [{
                        ...
                        type: 'text', id: 'url', label: C.url, required: true, onLoad: function() { this.allowOnChange = true; },
                        onKeyUp: function() {
                            var J = this;
                    ...
                    setup: function(E) { if (!this.getDialog().getContentElement('info', 'linkType')) this.getElement().show(); }
                ...
                padding: 0,
                children: [{
                    type: 'fieldset', id: 'selectAnchorText', label: D.selectAnchor,
                    setup: function(E) {
                        if (E.anchors.length > 0) this.getElement().show();
                ...
                setup: function(E) { if (!this.getDialog().getContentElement('info', 'linkType')) this.getElement().hide(); }
            }, {
                type: 'vbox', id: 'emailOptions', padding: 1,
                children: [{
                    type: 'text', id: 'emailAddress', label: D.emailAddress, required: true,
                    validate: function() {
                        var E = this.getDialog();
                    ...
                    type: 'text', id: 'emailSubject', label: D.emailSubject, setup: function(E) { if (E.email) this.setValue(E.email.subject); },
                    commit: function(E) {
                        if (!E.email) E.email = { };
                        E.email.subject = this.getValue();
                    }
                }, {
                    type: 'textarea', id: 'emailBody', label: D.emailBody, rows: 3, 'default': '', setup: function(E) { if (E.email) this.setValue(E.email.body); },
                    commit: function(E) {
                        if (!E.email) E.email = { };
            ...
            id: 'target', label: D.target, title: D.target,
            elements: [{
                type: 'hbox', widths: ['50%', '50%'],
                children: [{
                    type: 'select', id: 'linkTargetType', label: C.target, 'default': 'notSet', style: 'width : 100%;', items: [[C.notSet, 'notSet'], [D.targetFrame, 'frame'], [D.targetPopup, 'popup'], [C.targetNew, '_blank'], [C.targetTop, '_top'], [C.targetSelf, '_self'], [C.targetParent, '_parent']], onChange: c,
                    setup: function(E) {
        ...
        onOk: function() {
            ...
            if (G.target)
                if (G.target.type == 'popup') {
                    var U = ["window.open(this.href, '", G.target.name || '', "', '"], V = ['resizable', 'status', 'location', 'toolbar', 'menubar', 'fullscreen', 'scrollbars', 'dependent'], W = V.length, X = function(aj) { if (G.target[aj]) V.push(aj + '=' + G.target[aj]); };
            ...
            if (!this._.selectedElement) {
                ...
                if (ag == ah || G.type == 'email' && ah.indexOf('@') != -1) af.setHtml(G.type == 'email' ? G.email.address : E['data-cke-saved-href']);
                if (af.getAttribute('name')) af.addClass('cke_anchor');
                else af.removeClass('cke_anchor');
