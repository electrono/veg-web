    ...
    $.createLangSwitch = function(p) {
        ...
        var LanguageSwitch = {
            config: {
                ...
                data: { data: '' },
                dataType: 'json',
                baseURL: LanguageSwitchFilePath + "js/WebMethods.aspx/",
                method: "",
                url: "",
                ajaxCallMode: 0,
                arr: [],
            ...
            LoadFlags: function(data) {
                ...
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ baseURL: LanguageSwitchFlagPath, PortalID: parseInt(LanguageSwitch.config.PortalID) });
                $.ajax({
                    type: LanguageSwitch.config.type,
                    contentType: LanguageSwitch.config.contentType,
                    cache: LanguageSwitch.config.cache,
                        ...
                        $.each(data, function(index, item) {
                            if (_EnableCarousel.toLowerCase() == "false") {
                                if (_ListTypeBoth.toLowerCase() == "true") {
                                    ...
                                    if (_ListAlign.toLowerCase() == "h") {
                                        if (item.LanguageCode == LanguageSwitch.config.CultureCode) {
                                            html += '<li class="cssClassSelectedFlag cssClassLanguageName"  code="' + item.LanguageCode + '"><span>' + item.LanguageN + "<b>(" + item.NativeName + ")</b>" + '</span></li>';
                                        } else {
                                ...
                                if (item.LanguageCode == LanguageSwitch.config.CultureCode) {
                                    carousel += '<li class="cssClassSelectedFlag" title="' + item.LanguageName + '"><a href="#" code=' + item.L + '><img src=' + item.FlagPath + ' /></a></li>';
                                } else {
