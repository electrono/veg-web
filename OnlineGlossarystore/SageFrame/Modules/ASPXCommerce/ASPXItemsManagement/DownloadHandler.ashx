...
public class DownloadHandler : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            string action = context.Request.QueryString["action"] != null
                                ? context.Request.QueryString["action"].ToString()
                                : string.Empty;
            string path = context.Request.QueryString["path"] != null
                              ? context.Request.QueryString["path"].ToString()
                              : string.Empty;
                        ...
                        case "download":
                            try
                            {
                                ...
                                context.Response.ContentType =
                                    FileManagerHelper.ReturnExtension(file.Extension.ToLower());
    ...
    public void handleRequest(HttpContext context, FileInfo file_path)
    {
        ...
        writeJson(context,
                  new DownloadItemAttribInfo()
                      {FileName = filename.ToString(), FileType = filetype.ToString(), FileSize = filesize});
    }
