...
<script type="text/javascript">
    //var servicePath = "Modules/Admin/DetailsBrowse/ASPXCommerceWebService.asmx/";
    ...
    function LoadCategoryDetailStaticImage() {
        $('#imgLoader').attr('src', '' + aspxTemplateFolderPath + '/images/ajax-loader.gif');
    }

    function CategoryListingHideAll() {
        $("#divItemViewOptions").hide();
        $("#divSearchPageNumber").hide();
        //  $("#divShowGeneralSearchResult").hide();
    }

    function LoadAllCategoryContents() {
        $.ajax({
            type: "POST",
            url: aspxservicePath + "ASPXCommerceWebService.asmx/GetCategoryDetailsOptions",
            data: JSON2.stringify({ categorykey: categorykey, storeID: storeId, portalID: portalId, userName: userName, cultureName: cultureName }),
                    ...
                    $.each(msg.d, function(index, value) {
                        ...
                        if (index < 1) {
                            ...
                            if (value.CategoryImage != '') {
                                headerElements += "<img src='" + aspxRootPath + value.CategoryImage.replace("Small", "Large") + "' alt='" + value.CategoryName + "' title='" + value.CategoryName + "' />";
                            }
                            headerElements += "" + value.CategoryName + "";
                            $("#divHeader").html(headerElements);
                            catIds = catIds + '' + value.CategoryID + '' + ",";

                        } else {
                            if (catIds.indexOf('' + value.CategoryID + '') == -1 && value.ParentID == parentCatID) {
                                var childCategories = '<a href="' + aspxRedirectPath + 'category/' + value.CategoryName + '.aspx">' + value.CategoryName + '</a>';
...
//            }
    ...
    function AddToCartToJSCategoryDetail(itemId, itemPrice, itemSKU, itemQuantity) {
    ...
    function GridView() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultGrid").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
                // $(".cssClassAddtoCard").append(value.ItemID);
    ...
    function ListView() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultList").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
    ...
    function Grid2View() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultGrid2").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
                if ('<%= allowOutStockPurchase %>'.toLowerCase() == 'false') {
                    if (value.IsOutOfStock) {
                        $(".cssClassAddtoCard_" + value.ItemID + " span").html('Out Of Stock');
    ...
    function Grid3View() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultGrid3").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
            }
        });
    }
    ...
    function ProductGridView() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultProductGrid").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
                if ('<%= allowOutStockPurchase %>'.toLowerCase() == 'false') {
                    if (value.IsOutOfStock) {
                        $(".cssClassAddtoCard_" + value.ItemID + " span").html('Out Of Stock');
    ...
    function ListWithoutOptionsView() {
        ...
        $.each(arrResultToBind, function(index, value) {
            if (itemIds.indexOf('' + value.ItemID + '') == -1) {
                ...
                $("#scriptResultListWithoutOptions").tmpl(items).appendTo("#divShowCategoryItemsList");
                $('.cssClassFormatCurrency').formatCurrency({ colorize: true, region: '' + region + '' });
                if ('<%= allowOutStockPurchase %>'.toLowerCase() == 'false') {
                    if (value.IsOutOfStock) {
    ...
    <div class="cssClassProductsBox"> 
        ...
        <div class="cssClassAddtoCard_${itemID} cssClassAddtoCard">
            <div class="cssClassButtonWrapper"> 
                <a href="#" onclick=" AddToCartToJSCategoryDetail(${itemID},${price},${JSON2.stringify(sku)},${1}); "><span>Add to Cart</span></a>
            </div></div>  
    </div>   
</script>
...
<script id="scriptResultList" type="text/x-jquery-tmpl">
    ...
    <div class="cssClassProductListViewRight">
        ...
        <p class="cssClassListViewProductPrice">Price : <span><b><span class="cssClassFormatCurrency">${listPrice}</span></b> </span><span class="cssClassFormatCurrency">${price}</span></p>
        <div class="cssClassViewDetailsAddtoCart"> 
            ...
            <div class="cssClassAddtoCard_${itemID} cssClassAddtoCard">
                <div class="cssClassButtonWrapper">
                    <a href="#" onclick=" AddToCartToJSCategoryDetail(${itemID},${price},${JSON2.stringify(sku)},${1}); "><span>Add to Cart</span></a></div>
            </div>
        </div>
        <div class="cssClassClear"></div>
    </div>
        ...
        <div class="cssClassProductsGridInfo">
            ...
            <div class="cssClassButtonWrapper">
                <div class="cssClassWishListButton">
                    <button onclick=" AddToWishList(${itemID}); " id="addWishList" type="button"><span><span><span>+</span>Add to Wishlist</span></span></button>
                </div>
            </div>
            <div class="cssClassAddtoCard_${itemID} cssClassAddtoCard">
                <div class="cssClassButtonWrapper"> 
