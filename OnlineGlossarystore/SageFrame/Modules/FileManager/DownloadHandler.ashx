...
public class DownloadHandler : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        int index = lstFile.FindIndex(
            delegate(ATTFile obj) { return (obj.FileId == FileID); }
            );
    ...
    {
        return
            (FileManagerHelper.ReplaceBackSlash(
                Path.Combine(HttpContext.Current.Request.PhysicalApplicationPath.ToString(), filepath)));
    }
    ...
    public void DownloadSecureFile(HttpContext context, string filepath)
    {
        ...
        if (file.Exists)
        {
            ...
            context.Response.AddHeader("Content-Disposition",
                                       "attachment; filename=" + file.Name.Replace(' ', '_').Replace(".resources", ""));
