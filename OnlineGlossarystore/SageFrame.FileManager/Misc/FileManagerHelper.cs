...
namespace SageFrame.FileManager
{
         ...
         public static void TransferFile(string filePath, int fileId, int toFolderId, string toPath, int action, int mode,string fullFilePath,string fullFromPath,string fullToPath)
    {
        ...
        /// <param name="dictImages"></param>
         public static void ConstructHTMLString(bool IsZip, bool IsImg, int StorageLocation, string ext, string urlPath, string absolutePath, string downloadPath, string checkId, int folderId, ATTFile file, ref StringBuilder sb, string permission,Dictionary<string,string> dictImages)
         {
             switch (StorageLocation)
             {
                 case 0:
                     ...
                     if (IsZip)
                     {
                         sb.Append(AddExtension(ext));
                         sb.Append(AddCheckBox(checkId));
                         sb.Append(AddDownloadLink(downloadPath, folderId, file, permission));
         ...
         {
             return ("<li class=\"file ext_" + extension + "\">");
         }
         public static string AddCheckBox(string checkId)
         {
             return ("<span class=\"check\"><input type=\"checkbox\" id=" + checkId + "></span>");
         }
         ...
         public static string AddInfoSpan(ATTFile file, string attributeString, int mode)
         {
             ...
             {
                 case 2:
                     html = "<span class=\"info\">" + file.AddedOn + "||" + file.Size + "bytes" + "</span></span>";
                     break;
                 default:
                     html = "<span class=\"info\">" + file.AddedOn + "||" + file.Size + "bytes" + "||<span class=\"attr\">" + attributeString + "</span></span>";
                     break;
             }
             return html;
         }
         ...
         {
             if (permission == "edit")
                 return ("<a class=\"delete\" href=\"#\" rel=\"" + filePath + "\"><img src=\"" + image + "\"></a>");
             else
                 return ("");
         ...
         {
             if (permission == "edit")
                 return ("<span class=\"edit\"><a class=\"edit\" href=\"#\" rel=\"" + filePath + "\"><img src=\"" + image + "\"></a></span>");
             else
                 return ("");
         }
         public static string AddPreviewButton(string filePath, string image)
         {
             ...
             return ("<span class=\"preview\"><a class=\"preview\" href=\"" + filePath + "\" title=\"Preview\" rel=\"" + filePath + "\"><img src=\"" + image + "\"></a></span>");

         }
         public static string AddExtractButton(string filePath, string image, string permission)
         {
             if (permission == "edit")
                 return ("<span class=\"decompress\"><a class=\"decompress\" title=\"Extract\" href=\"#\" rel=\"" + filePath + "\"><img src=\"" + image + "\"></a></span>");
             else
                 return ("<span class=\"decompress\"></span>");
         }
